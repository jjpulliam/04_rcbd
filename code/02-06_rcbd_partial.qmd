...
#model assumptions are based on residuals ans not raw data becuase raw data does not change but you can change the models that you use 
```
...
If not it would have a "fan shape" such as having small residuals on the left side with increasing residuals as you move right.
...
Points in the center usually line up around the the center of the line while the tails do not. Data that is not normally distrubted it would be more S shaped.
...
The assumption is that the residuals have mean of 0, so if they are not that is a probelm. This data is fine.
...
It would be better fit if this line was more straight.

Just because something an outlier does not mean we just go delete that point. check your data- is it a data entry mistake? was there damage to that plot? if so, then maybe you delete that plot. 

He will run model with and without it, and if it does not change interpretation then you leave outlier and if it does change then you need think about what to do.


# h) Model means  

If data is balanced, arithmetic mean and model mean will be the same.
...
rcbd_means_all
```
the way you extract your means should affect the way you discuss your results. 
...
The adjustment of tukey test makes it harder for model to find differences, it makes it more restrictive. 

this makes it a data fram
...
here we are using two data frames so we specify this at geom level

fill has to be inside of aes 
...
ggsave("../output/rcbd_means_jjp.png")
```
